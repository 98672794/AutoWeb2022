...
def _indexIs0(v1,BassLan): 
    while True:
        ...
        if sel_AutoWebSetting == 1:
            if ATW2022._AutoWebKeySetting('','',v1) == 'True':    
                break   # 202203
        else:
            continue    # not 0 1 re
        break   # 出if0=修改
...
def _indexIs1(v1,BassLan): 
    ...
    # UserKeDemo = 自動覆客_OK2022._2022_AutoWhatsappReplyTextListKeyDemo   # in 自動覆客_OK2022 Reply demo
    UserKeDemoUrl = 'https://98672794.github.io/自動覆客/'

    UserKeDemo = "查詢','您好@+@請選','@@','query','Hello@+@Please select','@@','クエリ','こんにちは@+@選択してください','@@','쿼리','안녕하세요@+@선택해주세요','@@','查询','你好@+@请选择','@@"

    # 檢查 創 ATW User whatsapp自動客服機器人 Key 
    UserKeD3123emo = _GetAllUserKey(v1,BassLan,'WhatsappAutoReplyTextList',UserKeDemo)
    # 1 ATW Login wts
    自動覆客_OK2022.AdminLoginWTS()
    
    # 2 ATW 等客Loop
    自動覆客_OK2022._2022startToLoop(UserKeD3123emo)
